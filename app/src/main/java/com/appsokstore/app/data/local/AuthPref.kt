package com.appsokstore.app.data.local

import android.content.Context
import android.content.SharedPreferences
import com.appsokstore.app.data.model.ActionState
import com.appsokstore.app.data.model.AuthUser
import com.appsokstore.app.util.getObject
import com.appsokstore.app.util.putObject
...
class AuthPref (val context:Context){
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser>{
        ...
        return if(user == null){
            ...
            ActionState( message = "Email dan passwoard tidak boleh kosong", isSuccess = false)
        } else if (user.email == email && user.password == password){
            isLogin = true
            ActionState(authUser, message = "Anda berhasil login")
        } else {
            ActionState(message = "Email atau Passwoard salah", isSuccess = false)
        }
    }
